...
class Spider(Spider):
    ...
    def playerContent(self, flag, id, vipFlags):
        # 使用当前最优域名动态构造播放链接
        optimal_host = self.gethost() if hasattr(self, 'gethost') else self.hsot
        play_url = f"{optimal_host}{id}"
        
        # 更新请求头中的referer字段
        player_headers = self.pheader.copy()
        player_headers['referer'] = f"{optimal_host}/"
